...
q = IsolationForest(n_estimators=100)
q.fit(data)
res = q.predict(data)
outlier_indices = data.index[res == -1].tolist()
outlier_values = data.loc[outlier_indices]
data.drop(outlier_indices, axis="rows", inplace=True)
outlier_values
````
### sklearn.multioutput
````
...
graph.render("iris_tree.pdf")  # создаст файл iris_tree.pdf


from sklearn.inspection import partial_dependence, PartialDependenceDisplay
result = PartialDependenceDisplay.from_estimator(model, X_test, features=[0,1,2,3,4,5,6,7,8], feature_names=data.columns[1:-1])


from sklearn.covariance import EllipticEnvelope
detector = EllipticEnvelope(support_fraction=0.75).fit(X_train)
...
````
pip install shap
exc = shap.TreeExplainer(model)
shap_values = exc.shap_values(data)
